...
自家席位受益 = 工程面。**混合面**（一批里两者都真交付）= 单列，不硬分。

**两件仪器不等价，别把一件当另一件**（评审实测：十批业务/工程交替 = 5:5，连续规则一次都不响）：

- **比值 = 目标，不是自动闸**：8:2 由复盘记账呈现（每单元标面、混合单列），越线与否**归主理人判**——
  面的归类是自声明的，自动分类已知会误判（运行时代码就住在产品目录里）。
- **连续规则 = 唯一自动停**，且它只治最坏那一种形态（长时间零对外价值），不代表比值达标：
  **纯工程面**批次连续计数，业务面与混合面各自清零；连续 2 批 → 第 3 批开工前 STOP-and-report。
  比值长期偏离但从不连续 = 自动闸看不见，靠复盘那一栏被人看见。
