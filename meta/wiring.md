...
4. **配置文件不携带文档字段；多实现的接线逐实现实测。** 消费配置的解析器有严格与宽容两种脾气，
   严格者遇未知字段（哪怕 `_comment`）会**静默禁用整个文件**——文档写 README，JSON 只放机器要读的。
   同一份接线给 N 个工具消费时，"在 A 上实测过" 对 B 是零证据：接线验证按**消费者矩阵**逐个真跑。
   实证（2026-07-05）：某 agent 的 hooks 模板顶层带 `_comment`，被严格解析的消费端静默整文件不加载，
   监控 hook **自诞生起零生效**、降级路径掩护了整个生命周期——直到三消费端 e2e 矩阵门首跑才现形；
   同一批还抓到另一消费端首启信任门吃指令的差异。两层修复：字段清除 + 「模板顶层只许功能键」机械门。
