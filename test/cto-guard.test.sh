#!/usr/bin/env bash
# cto-guard.sh — single consolidated enforcement hook, dispatched on hook_event_name + tool_name.
...
# Deny = exit 2 + stderr ; reminder = stdout + exit 0 ; everything else passes silently exit 0.
set -u
cd "$(dirname "$0")"
. ./lib-testkit.sh

GUARD="../skills/cto-orchestration/references/agent-watch/cto-guard.sh"
...
run() { # $1 event  $2 tool  $3 val  -> sets OUT(stdout) ERR(stderr) RC
  ...
  OUT="$(mkjson "$1" "$2" "$3" | bash "$GUARD" 2>"$tmpe")"; RC=$?
...
# --- PostToolUse·Agent|Task: browser/E2E subagent -> REMIND (stdout, exit 0) ---
run PostToolUse Agent 'run playwright E2E against localhost:3000'
chk_eq "browser Agent exit 0" 0 "$RC"; chk_contains "browser Agent reminder" "BLACK-HOLE" "$OUT"
run PostToolUse Task 'take a screenshot of the dev server'
chk_contains "browser Task reminded" "BLACK-HOLE" "$OUT"
