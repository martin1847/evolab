...
chk_contains "protocol freezes instrument identity per batch" "禁与旧批合并" "$meas_body"
chk_contains "protocol keeps the denominator preregistered" "分母恒为预注册样本量" "$meas_body"
chk_contains "protocol blinds the judge" "不含臂身份与预期结论" "$meas_body"
chk_contains "protocol demotes self-judging to diagnostics" "不得单独支撑 headline" "$meas_body"
chk_contains "protocol keeps rerun discipline single-sourced" "整批环境性失效" "$meas_body"
chk_contains "incomplete runs never headline" "INCOMPLETE、禁出 headline" "$meas_body"
chk_contains "no single-point headline" "禁单点结论" "$meas_body"
chk_contains "self-report is never evidence" "被测自报永不作证据" "$meas_body"
