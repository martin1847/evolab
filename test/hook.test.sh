#!/usr/bin/env bash
# memory-discipline-hook.sh — shared PostToolUse hook core (Claude Code + codex).
# Drives the script with synthetic stdin and asserts the match matrix:
# memory/*.md (non-index) -> reminder; everything else -> silent.
set -u
cd "$(dirname "$0")"
. ./lib-testkit.sh

HOOK="../skills/repo-governance-bootstrap/references/memory-discipline-hook.sh"

echo "== memory-discipline-hook.sh =="

if ! command -v jq >/dev/null 2>&1; then
  echo "    jq not on PATH — hook test skipped"; exit 0
fi

# feed <file_path-json-fragment> on stdin, capture stdout
