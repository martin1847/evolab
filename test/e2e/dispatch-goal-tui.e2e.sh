#!/usr/bin/env bash
# dispatch-goal-tui.e2e.sh — LIVE closed-loop for the TUI-lane fused `dispatch --goal` flow.
# BEST-EFFORT since the 2026-07-12 default flip (headless exec is the gated lane —
# dispatch-goal.e2e.sh): NOT in the pre-release gate set; run manually with E2E_TUI=1 when
# working on TUI adaptation debt. Known-red 2026-07-12: omp 16.4.4 silently stopped loading
# `--hook` modules (v1.4.0 hook same-red = environment drift) and the codex 0.144.1 leg
# never executed the goal. claude leg green.
# Matrix: claude + omp + codex (a leg skips only if its CLI is absent).
...
echo "== dispatch-goal-tui.e2e (best-effort TUI matrix; E2E_TUI=1 to run) =="
[ "${E2E_TUI:-0}" = "1" ] || { echo "  SKIP: TUI lane is best-effort — set E2E_TUI=1 to run"; exit 0; }
...
run_leg() {  # run_leg <agent> [extra agent flags...]
  local agent="$1"; shift
...
EOF
  ...
  out="$(cd "$WT" && DISPATCH_TUI=1 AGENT_WATCH_DIR="$TMPRUN" timeout 240 \
  ...
  # ③ hook lifecycle: agent's own hook MUST write the sentinel (claude settings.json /
  #    omp --hook / codex .codex/hooks.json). History: codex was silent since birth because the
  #    SHIPPED template carried a top-level `_comment` key — codex strict-parses hooks.json and an
  #    unknown field silently disables the whole file (root-caused + fixed 2026-07-05; the
  #    top-level-keys gate in skill-polish-check now guards the template).
