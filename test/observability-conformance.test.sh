...
for token in tenant.id service.namespace gen_ai.prompt.name gen_ai.tool.call.id im.agent.role im.agent.invocation.id im.workflow.phase qae.metadata.actor observability_conformance_command conformance-profile-v2.json; do
  grep -Fq "$token" "$SKILL"
  grep -Fq "$token" "$STANDARD"
done

cat >"$TMP/good.json" <<'JSON'
{"resource":{"service.name":"svc","service.version":"tag","service.namespace":"product","deployment.environment.name":"development","k8s.namespace.name":"runtime"},"tenant":{"verified":false},"processes":[{"name":"api","instrumentation_bootstrapped":true},{"name":"worker","instrumentation_bootstrapped":true}],"gen_ai_enabled":true,"content_capture_allowed":false,"metric_label_keys":["im.agent.role","im.workflow.phase"],"static_guard":{"semantic_producers":["litellm_otel_v2"],"vendor_or_legacy_paths":[],"arbitrary_metadata_flatteners":[]},"tool_envelopes":[{"tool_call_id":"call-1"}],"spans":[{"trace_id":"trace","span_id":"domain","parent_span_id":null,"role":"domain","ended":true,"attributes":{"gen_ai.operation.name":"invoke_workflow","im.agent.role":"planner","im.agent.invocation.id":"inv-1","im.workflow.phase":"plan"}},{"trace_id":"trace","span_id":"worker","parent_span_id":"domain","role":"invocation","ended":true,"async_boundary":"worker_callback","context_restored":true,"attributes":{"gen_ai.operation.name":"invoke_agent"}},{"trace_id":"trace","span_id":"llm","parent_span_id":"worker","role":"inference","ended":true,"streaming":true,"stream_completed":true,"status":"success","attributes":{"gen_ai.operation.name":"chat","gen_ai.provider.name":"provider","gen_ai.request.model":"model","gen_ai.prompt.name":"prompt","gen_ai.prompt.version":"1"}}]}
JSON
...
for mutation in parent field agent_field metric_label tenant content_dev_flag bootstrap empty resource_tenant span_tenant legacy_qae placeholder stream_completed_status stream_incomplete_status async_missing async_restore async_parent async_trace producer vendor_path metadata_flatten malformed trace; do
...
elif m=="agent_field": p["spans"][0]["attributes"].pop("im.workflow.phase")
elif m=="metric_label": p["metric_label_keys"].append("im.agent.invocation.id")
...
elif m=="legacy_qae": p["spans"][0]["attributes"]["qae.metadata.actor"]="planner"
...
elif m=="producer": p["static_guard"]["semantic_producers"].append("custom_callback")
...
echo "observability conformance: 2 positive profiles + 23 mutations clean"
