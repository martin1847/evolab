#!/usr/bin/env bash
# Copilot primitives (B1, 2026-09-25): `agentctl steer --thread` and `agentctl settings`.
#
# WHAT THESE VERBS ARE: the only two things agentctl can do to a session it did NOT start —
...
copilot_setup
out="$(ac steer --thread "$THREAD" -m "no daemon here" 2>&1)"; rc=$?
...
out="$(ac steer --thread "$THREAD" -m "daemon is up" 2>&1)"; rc=$?
...
ac steer --thread "$THREAD" -m "tagged" --tag montior >/dev/null 2>&1
...
out="$(ac steer --thread "$THREAD" -m "who owns this thread" 2>&1)"; rc=$?
...
copilot_setup
out="$(ac steer --thread "$MISSING_THREAD" -m "nobody home" 2>&1)"; rc=$?
...
out="$(ac steer --thread "not-a-uuid" -m "x" 2>&1)"; rc=$?
...
PY
out="$(ac steer --thread "$CLAUDE_NAME" --engine claude -m "which one" 2>&1)"; rc=$?
...
for uuid in "$CLAUDE_SID" "0199dd44-5555-4666-8777-888899990000"; do
  out="$(ac steer --thread "$uuid" --engine claude -m "by uuid" 2>&1)"; rc=$?
...
with open(os.path.join(home, ".claude", "sessions", "4444.json"), "w", encoding="utf-8") as fh:
    json.dump({"sessionId": "0199ee55-6666-4777-8888-999900001111",
               "cwd": os.path.join(home, "work"), "status": "shell"}, fh)
PY
out="$(ac steer --thread "0199ee55-6666-4777-8888-999900001111" --engine claude -m "no name" 2>&1)"
rc=$?
...
out="$(ac steer --thread "zz-copilot" --engine claude -m "prefix only" 2>&1)"; rc=$?
...
copilot_setup
out="$(ac steer --thread "$CLAUDE_NAME" --engine claude -m "reply PONG" 2>&1)"; rc=$?
...
: > "$STUB_ARGV_LOG"
out="$(ac steer --thread "$CLAUDE_SID" --engine claude -m "by id" 2>&1)"; rc=$?
chk_eq "D a sessionId addresses the same seat" 15 "$rc"
export FAKE_CLAUDE_MODE=fail
out="$(ac steer --thread "$CLAUDE_NAME" --engine claude -m "held" 2>&1)"; rc=$?
...
out="$(ac steer --thread "$THREAD" -m "confirm me" --confirm 2>&1)"; rc=$?
...
out="$(ac steer --thread "$THREAD" -m "live order" --confirm 2>&1)"; rc=$?
...
out="$(ac steer --thread "$THREAD" -m "no turn follows" --confirm 2>&1)"; rc=$?
...
out="$(ac steer --thread "$THREAD" -m "damaged record" --confirm 2>&1)"; rc=$?
...
out="$(ac steer --thread "$THREAD" -m "record gone" --confirm 2>&1)"; rc=$?
...
out="$(ac steer --thread "$THREAD" -m "old news" --confirm 2>&1)"; rc=$?
...
out="$(ac steer --thread "$THREAD" -m "budget already spent" --confirm 2>&1)"; rc=$?
...
late_writer=$!
out="$(ac steer --thread "$THREAD" -m "late pair" --confirm 2>&1)"; rc=$?
...
out="$(ac steer --thread "$CLAUDE_NAME" --engine claude -m "reply PONG" --confirm 2>&1)"; rc=$?
...
out="$(ac steer --thread "$CLAUDE_NAME" --engine claude -m "silent target" --confirm 2>&1)"; rc=$?
...
copilot_setup
out="$(ac settings --thread "$THREAD" --engine codex 2>&1)"; rc=$?
...
out="$(ac settings --thread "$CLAUDE_NAME" --engine claude 2>&1)"; rc=$?
...
out2="$(ac steer --thread "$THREAD" -m "cannot deliver" 2>&1)"; rc2=$?
...
out="$(ac settings --thread "$THREAD" 2>&1)"; rc=$?
...
# ─────────────────────────────────────────────────────────────────────────────────────────
echo "== I: enginectl — one file, three names, engine prefilled =="
copilot_setup
ENGINECTL="$(dirname "$AGENTCTL")/enginectl"
ln -sf "$ENGINECTL" "$BIN/codexctl"
ln -sf "$ENGINECTL" "$BIN/claudectl"
ln -sf "$ENGINECTL" "$BIN/ompctl"
ln -sf "$ENGINECTL" "$BIN/wrongctl"
out="$(codexctl steer --thread "$THREAD" -m "through the alias" 2>&1)"; rc=$?
chk_eq "I codexctl steer --thread prefills --engine codex" 15 "$rc"
chk_contains "I and the codex CLI really got it" \
  "--message [copilot] through the alias" "$(cat "$STUB_ARGV_LOG")"
: > "$STUB_ARGV_LOG"
out="$(claudectl steer --thread "$CLAUDE_NAME" -m "peer via alias" 2>&1)"; rc=$?
chk_eq "I claudectl routes the same verb to the OTHER engine" 15 "$rc"
chk_contains "I proven by which binary was invoked" "--name copilot-copilot" \
  "$(cat "$STUB_ARGV_LOG")"
out="$(claudectl settings --thread "$CLAUDE_NAME" 2>&1)"; rc=$?
chk_eq "I settings is prefilled too" 0 "$rc"
chk_contains "I with the claude reading" "model=claude-haiku-4" "$out"
# an explicit --engine always wins over the name that was invoked
out="$(claudectl settings --thread "$THREAD" --engine codex 2>&1)"; rc=$?
chk_eq "I an explicit --engine is never overridden" 0 "$rc"
chk_contains "I and it read the codex record" "source=turn_context@3" "$out"
# the session form has no engine flag at all: it must pass through untouched
out="$(ompctl status nosuchseat 2>&1)"; rc=$?
chk_eq "I a passthrough verb is still agentctl's" 1 "$rc"
chk_contains "I with agentctl's own refusal" "unknown session" "$out"
out="$(wrongctl status x 2>&1)"; rc=$?
chk_eq "I a name that encodes no engine is refused" 1 "$rc"
chk_contains "I and says which names are valid" "codexctl, claudectl or ompctl" "$out"
copilot_teardown
