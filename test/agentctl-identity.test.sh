...
# ─────────────────────────────────────────────────────────────────────────────────────────
echo "== R1-F1: publish and every identity transition share ONE lock (no resurrection) =="
# Reviewer's blocker probe, verbatim method: pause the production _atomic_json inside a
# publish and let a concurrent replace mint B. Pre-fix the paused publisher wrote back the
# record it had loaded before the pause and resurrected A as the ACTIVE identity.
F1DIR="$(mktemp -d "${TMPDIR:-/tmp}/ws1-f1.XXXXXX")"
printf 'engine=omp\ncwd=%s\n' "$F1DIR" > "$F1DIR/f1.duplex.meta"
f1out="$(python3 - "$AW_DIR/identity.py" "$F1DIR" <<'PYEOF'
import importlib.util as u, sys, threading, time
spec = u.spec_from_file_location("idm", sys.argv[1]); m = u.module_from_spec(spec)
assert spec.loader; spec.loader.exec_module(m)
run = sys.argv[2]

a_store = m.IdentityStore(run, "f1")
a_rec = a_store.transition("start")
a_attempt = a_rec["attemptId"]
armed = a_store.token()

original = m.IdentityStore._atomic_json
started, release, once = threading.Event(), threading.Event(), {"hit": False}


def paused(self, path, payload):
    if not once["hit"]:
        once["hit"] = True
        started.set()
        release.wait(10)
...
m.IdentityStore._atomic_json = paused
box = {}
...
def replacer():
    box["replace"] = m.IdentityStore(run, "f1").transition("replace")["attemptId"]


pub = threading.Thread(target=publisher); pub.start()
started.wait(10)
rep = threading.Thread(target=replacer); rep.start()
time.sleep(0.5)          # pre-fix window: the replace lands mid-publish
release.set()
pub.join(20); rep.join(20)

m.IdentityStore._atomic_json = original
active, status = m.IdentityStore(run, "f1").load()
b_attempt = box.get("replace", "")
mstatus, marker = m.IdentityStore(run, "f1").read_terminal_marker()
name = {a_attempt: "A", b_attempt: "B"}
print("publish_class", (box.get("publish") or ("?",))[0])
print("active_after", name.get((active or {}).get("attemptId"), "other"))
print("marker_attempt", name.get(m.IdentityStore.marker_stamp(marker).get("attemptId"), "none"))
print("resurrected_old_attempt", (active or {}).get("attemptId") == a_attempt)
PYEOF
)"
f1line() { printf '%s\n' "$f1out" | sed -n "s/^$1 //p"; }
chk_eq "F1 arrange: the concurrent replace minted B" 1 \
  "$([ -n "$(f1line active_after)" ] && echo 1 || echo 0)"
chk_eq "F1 DAMAGE ORACLE: a paused publish never resurrects the armed attempt" "False" \
  "$(f1line resurrected_old_attempt)"
chk_eq "F1 B stays authoritative after the serialized publish" "B" "$(f1line active_after)"
chk_not_contains "F1 the marker never carries B's authority from an A publisher" "marker_attempt B" "$f1out"

# paired green / control: with the replace landing BEFORE the publish decides, the publish is
# refused, nothing is published, and the refused publish must not mutate the record at all
printf 'engine=omp\ncwd=%s\n' "$F1DIR" > "$F1DIR/f2.duplex.meta"
f1b="$(python3 - "$AW_DIR/identity.py" "$F1DIR" <<'PYEOF'
import importlib.util as u, json, os, sys
spec = u.spec_from_file_location("idm", sys.argv[1]); m = u.module_from_spec(spec)
assert spec.loader; spec.loader.exec_module(m)
run = sys.argv[2]
store = m.IdentityStore(run, "f2")
...
f2line() { printf '%s\n' "$f1b" | sed -n "s/^$1 //p"; }
chk_eq "F1 REFUSAL: a replace that lands first makes the publish REFUSE" "STALE-ATTEMPT" "$(f2line class)"
chk_eq "F1 REFUSAL: the refused publish writes no marker" "False" "$(f2line marker_exists)"
chk_eq "F1 REFUSAL: the refused publish leaves the active record byte-identical" "True" \
  "$(f2line record_untouched)"
rm -rf "$F1DIR"
...
echo "== R1-F4: event sameness is a per-attempt MONOTONIC high-watermark, not a ring =="
# Reviewer's probe: the 128-entry ring forgot old keys, so replaying sequence 0 after applying
# 0..128 became "new" again and transitioned a second time. The amended contract forbids
# eviction-based dedupe: accept only seq > watermark, for the attempt's whole lifetime.
F4DIR="$(mktemp -d "${TMPDIR:-/tmp}/ws1-f4.XXXXXX")"
printf 'engine=omp\ncwd=%s\n' "$F4DIR" > "$F4DIR/f4.duplex.meta"
f4out="$(python3 - "$AW_DIR/identity.py" "$F4DIR" <<'PYEOF'
import importlib.util as u, sys
spec = u.spec_from_file_location("idm", sys.argv[1]); m = u.module_from_spec(spec)
assert spec.loader; spec.loader.exec_module(m)
store = m.IdentityStore(sys.argv[2], "f4")
attempt = store.transition("start")["attemptId"]
for seq in range(129):                       # 0..128 — one more than the old ring capacity
    store.apply_event(attempt, seq)
after_fill = store.load()[0]["stateVersion"]
replayed = store.apply_event(attempt, 0)     # the evicted key: must stay a duplicate
after_replay = store.load()[0]["stateVersion"]
print("evicted_replay_applied", replayed)
print("stable_state_version", after_fill == after_replay)
print("state_version", after_replay)
# paired green: a strictly increasing sequence still applies, and every intermediate seq
# at or below the watermark stays a no-op
print("next_seq_applied", store.apply_event(attempt, 129))
after_next = store.load()[0]["stateVersion"]
print("one_more_transition", after_next - after_replay == 1)
print("mid_seq_replay_applied", store.apply_event(attempt, 64))
# a NEW attempt starts its own watermark: seq 0 must be live again for attempt B
attempt_b = store.transition("replace")["attemptId"]
print("new_attempt_seq0_applied", store.apply_event(attempt_b, 0))
PYEOF
)"
f4line() { printf '%s\n' "$f4out" | sed -n "s/^$1 //p"; }
chk_eq "F4 DAMAGE ORACLE: 129 events plus one replay is still 129 transitions" 129 "$(f4line state_version)"
chk_eq "F4 DAMAGE ORACLE: replaying an evicted seq is a no-op" "False" "$(f4line evicted_replay_applied)"
chk_eq "F4 DAMAGE ORACLE: and the state version does not move" "True" "$(f4line stable_state_version)"
chk_eq "F4 any seq at or below the watermark stays a duplicate" "False" "$(f4line mid_seq_replay_applied)"
chk_eq "F4 PAIRED GREEN: a strictly increasing seq still applies" "True" "$(f4line next_seq_applied)"
chk_eq "F4 PAIRED GREEN: and moves the version by exactly one" "True" "$(f4line one_more_transition)"
chk_eq "F4 PAIRED GREEN: a new attempt gets a fresh watermark (seq 0 live again)" "True" \
  "$(f4line new_attempt_seq0_applied)"
rm -rf "$F4DIR"
...
# ─────────────────────────────────────────────────────────────────────────────────────────
echo "== R2-G1: cleanup must never unlink the lock (one session = one lock inode) =="
# Reviewer's blocker probe: clear() deleted the coordination inode without taking it, so a
# process paused inside its critical section held one inode while the next transition created
# and locked a DIFFERENT one. Both believed they held "the" session lock, and the paused
# publisher then wrote its stale record back over the newer attempt.
G1DIR="$(mktemp -d "${TMPDIR:-/tmp}/ws1-g1.XXXXXX")"
printf 'engine=omp\ncwd=%s\n' "$G1DIR" > "$G1DIR/g1.duplex.meta"
g1out="$(python3 - "$AW_DIR/identity.py" "$G1DIR" <<'PYEOF'
import importlib.util as u, os, sys, threading, time
spec = u.spec_from_file_location("idm", sys.argv[1]); m = u.module_from_spec(spec)
assert spec.loader; spec.loader.exec_module(m)
run = sys.argv[2]

a_store = m.IdentityStore(run, "g1")
a_attempt = a_store.transition("start")["attemptId"]
armed = a_store.token()
ino_before = os.stat(a_store.lock_path).st_ino
...
def stop_then_restart():
    # exactly what `agentctl stop` + a same-name `agentctl start` do
    m.IdentityStore(run, "g1").clear()
    box["b"] = m.IdentityStore(run, "g1").transition("start")["attemptId"]


pub = threading.Thread(target=publisher); pub.start()
started.wait(10)
cycle = threading.Thread(target=stop_then_restart); cycle.start()
time.sleep(0.5)          # pre-fix window: clear() + the new attempt land mid-publish
release.set()
pub.join(20); cycle.join(20)

m.IdentityStore._atomic_json = original
final = m.IdentityStore(run, "g1")
active, _status = final.load()
b_attempt = box.get("b", "")
name = {a_attempt: "A", b_attempt: "B"}
ino_after = os.stat(final.lock_path).st_ino if os.path.exists(final.lock_path) else 0
print("lock_inode_stable", ino_before == ino_after)
print("active_after", name.get((active or {}).get("attemptId"), "other"))
print("resurrected_old_attempt", (active or {}).get("attemptId") == a_attempt)
print("newer_record_stayed_authoritative", (active or {}).get("attemptId") == b_attempt)
PYEOF
)"
g1line() { printf '%s\n' "$g1out" | sed -n "s/^$1 //p"; }
chk_eq "G1 DAMAGE ORACLE: the lock inode survives clear() (never unlinked)" "True" \
  "$(g1line lock_inode_stable)"
chk_eq "G1 DAMAGE ORACLE: a stop+restart cycle never resurrects the paused attempt" "False" \
  "$(g1line resurrected_old_attempt)"
chk_eq "G1 the newer attempt stays authoritative" "True" \
  "$(g1line newer_record_stayed_authoritative)"
chk_eq "G1 and it is the one on disk" "B" "$(g1line active_after)"
rm -rf "$G1DIR"

echo "== R2-G2: a corrupt seqWatermark is IDENTITY-UNKNOWN, never reset-and-continue =="
# Reviewer's probe: apply seq 0, then corrupt ONLY the persisted watermark to the string
# "broken". load() still said ok and apply_event mapped the malformed value to -1, so the
# already-applied event replayed and the state version moved a second time.
G2DIR="$(mktemp -d "${TMPDIR:-/tmp}/ws1-g2.XXXXXX")"
printf 'engine=omp\ncwd=%s\n' "$G2DIR" > "$G2DIR/g2.duplex.meta"
g2out="$(python3 - "$AW_DIR/identity.py" "$G2DIR" <<'PYEOF'
import importlib.util as u, json, sys
spec = u.spec_from_file_location("idm", sys.argv[1]); m = u.module_from_spec(spec)
assert spec.loader; spec.loader.exec_module(m)
store = m.IdentityStore(sys.argv[2], "g2")
attempt = store.transition("start")["attemptId"]
print("first_apply", store.apply_event(attempt, 0))
before = json.load(open(store.path, encoding="utf-8"))
# corrupt ONLY the watermark, leaving every other field valid
broken = dict(before, seqWatermark="broken")
open(store.path, "w", encoding="utf-8").write(json.dumps(broken, sort_keys=True) + "\n")
print("load_status_after_corruption", store.load()[1])
try:
    applied = store.apply_event(attempt, 0)
    print("replay_applied", applied)
except m.IdentityPersistError as exc:
    print("replay_applied refused")
    print("refusal_names_watermark", "seqWatermark" in str(exc))
after = json.load(open(store.path, encoding="utf-8"))
print("record_untouched", after == broken)
print("state_version", before["stateVersion"], after["stateVersion"])
PYEOF
)"
g2line() { printf '%s\n' "$g2out" | sed -n "s/^$1 //p"; }
chk_eq "G2 arrange: the first event applied normally" "True" "$(g2line first_apply)"
chk_eq "G2 DAMAGE ORACLE: a corrupt watermark makes the record CORRUPT" "corrupt" \
  "$(g2line load_status_after_corruption)"
chk_eq "G2 DAMAGE ORACLE: the already-applied event is NOT replayed" "refused" \
  "$(g2line replay_applied)"
chk_eq "G2 the refusal names the offending field" "True" "$(g2line refusal_names_watermark)"
chk_eq "G2 DAMAGE ORACLE: and the record is left exactly as found" "True" \
  "$(g2line record_untouched)"
chk_eq "G2 the state version did not move" "1 1" "$(g2line state_version)"
rm -rf "$G2DIR"
...
# Reviewer's probe: the schema accepted every strict int while a new attempt starts at
# watermark -1, so the FIRST validly typed negative sequence was called an already-covered
# duplicate even though no event had been applied. Silently calling an unseen event a duplicate
# is not one of the allowed outcomes; the amended contract makes seq a NON-NEGATIVE int.
G3DIR="$(mktemp -d "${TMPDIR:-/tmp}/ws1-g3.XXXXXX")"
printf 'engine=omp\ncwd=%s\n' "$G3DIR" > "$G3DIR/g3.duplex.meta"
g3out="$(python3 - "$AW_DIR/identity.py" "$G3DIR" <<'PYEOF'
import importlib.util as u, sys
spec = u.spec_from_file_location("idm", sys.argv[1]); m = u.module_from_spec(spec)
assert spec.loader; spec.loader.exec_module(m)
store = m.IdentityStore(sys.argv[2], "g3")

attempt = store.transition("start")["attemptId"]
before = store.load()[0]["stateVersion"]
try:
    print("negative_first_applied", store.apply_event(attempt, -5))
except m.IdentityPersistError as exc:
    print("negative_first_applied refused")
    print("refusal_names_negative", "non-negative" in str(exc))
rec = store.load()[0]
print("version_delta", rec["stateVersion"] - before)
print("watermark_untouched", rec["seqWatermark"] == m.WATERMARK_UNSET)
# the schema gate must reject it before classification too
marker = {"ts": "2026-08-04T00:00:00Z", "rc": 0, "deliverable": "",
          "identity": {"attemptId": attempt, "processIncarnation": "1@x", "seq": -1}}
ok, detail = m.IdentityStore.validate_marker_schema(marker)
print("schema_accepts_negative", ok)
print("schema_detail_names_negative", "non-negative" in detail)
# paired green: seq 0 is a legitimate FIRST event
print("zero_first_applied", store.apply_event(attempt, 0))
print("zero_version_delta", store.load()[0]["stateVersion"] - before)
PYEOF
)"
g3line() { printf '%s\n' "$g3out" | sed -n "s/^$1 //p"; }
chk_eq "G3 DAMAGE ORACLE: a negative first seq is refused, not called a duplicate" "refused" \
  "$(g3line negative_first_applied)"
chk_eq "G3 the refusal names the non-negative rule" "True" "$(g3line refusal_names_negative)"
chk_eq "G3 DAMAGE ORACLE: it applies no transition" 0 "$(g3line version_delta)"
chk_eq "G3 and leaves the watermark unset" "True" "$(g3line watermark_untouched)"
chk_eq "G3 the marker schema gate rejects it before classification" "False" \
  "$(g3line schema_accepts_negative)"
chk_eq "G3 the schema reason names the rule" "True" "$(g3line schema_detail_names_negative)"
chk_eq "G3 PAIRED GREEN: seq 0 is a legitimate first event" "True" "$(g3line zero_first_applied)"
chk_eq "G3 PAIRED GREEN: and it moves the version by exactly one" 1 "$(g3line zero_version_delta)"
rm -rf "$G3DIR"
...
# Reviewer's blocker probe: clear()'s exception handler assumed that an unopenable lock PATH
# means nobody holds the lock. False — a publisher can already hold an open fd while the path
# becomes unopenable (mode 000). clear() then ran _clear_tree() with no lock at all, and the
# releasing publisher recreated the identity it had just deleted.
H1DIR="$(mktemp -d "${TMPDIR:-/tmp}/ws1-h1.XXXXXX")"
printf 'engine=omp\ncwd=%s\n' "$H1DIR" > "$H1DIR/h1.duplex.meta"
h1out="$(python3 - "$AW_DIR/identity.py" "$H1DIR" <<'PYEOF'
import importlib.util as u, os, sys, threading, time
spec = u.spec_from_file_location("idm", sys.argv[1]); m = u.module_from_spec(spec)
assert spec.loader; spec.loader.exec_module(m)
run = sys.argv[2]

a = m.IdentityStore(run, "h1")
a_attempt = a.transition("start")["attemptId"]
armed = a.token()
...
pub = threading.Thread(target=publisher); pub.start()
started.wait(10)
# the holder keeps its fd; the PATH becomes unopenable underneath it
os.chmod(a.lock_path, 0o000)
try:
    m.IdentityStore(run, "h1").clear()
    print("clear_result returned")
except m.IdentityPersistError as exc:
    print("clear_result refused")
    print("refusal_names_lock", "lock" in str(exc).lower())
print("record_survived_clear", os.path.exists(a.path))
release.set()
pub.join(20)
m.IdentityStore._atomic_json = original
os.chmod(a.lock_path, 0o600)

final = m.IdentityStore(run, "h1")
rec, status = final.load()
mstatus, marker = final.read_terminal_marker()
print("publish_class", (box.get("publish") or ("?",))[0])
print("active_is_A", (rec or {}).get("attemptId") == a_attempt)
print("marker_status", mstatus)
print("marker_is_A", m.IdentityStore.marker_stamp(marker).get("attemptId") == a_attempt)
# paired green: with the lock openable again, cleanup works exactly as before
final.clear()
print("paired_clear_removed_record", not os.path.exists(final.path))
print("paired_lock_still_there", os.path.exists(final.lock_path))
PYEOF
)"
h1line() { printf '%s\n' "$h1out" | sed -n "s/^$1 //p"; }
chk_eq "H1 DAMAGE ORACLE: cleanup REFUSES when the lock cannot be acquired" "refused" \
  "$(h1line clear_result)"
chk_eq "H1 the refusal names the lock" "True" "$(h1line refusal_names_lock)"
chk_eq "H1 DAMAGE ORACLE: the record is untouched by the refused cleanup" "True" \
  "$(h1line record_survived_clear)"
chk_eq "H1 the holder's publish still completed normally" "OK" "$(h1line publish_class)"
chk_eq "H1 state stays coherent: the active record is still A" "True" "$(h1line active_is_A)"
chk_eq "H1 and A's marker is intact, not half-cleared" "ok" "$(h1line marker_status)"
chk_eq "H1 the marker still carries A" "True" "$(h1line marker_is_A)"
chk_eq "H1 PAIRED GREEN: with the lock free, cleanup removes the record" "True" \
  "$(h1line paired_clear_removed_record)"
chk_eq "H1 PAIRED GREEN: and still never unlinks the lock (R2 rule holds)" "True" \
  "$(h1line paired_lock_still_there)"
rm -rf "$H1DIR"
