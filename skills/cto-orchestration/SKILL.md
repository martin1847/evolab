...
| 读密集、结论小的取证 | Agent subagent（显式 model 档）；多来源 / 多镜头 → 宿主 Workflow 工具扇出 | 主上下文亲读 |
...
- brief 冷上下文，不喂实现者结论；只审主路径 + 零变异 + 已知阳性（边界口径归 review-brief-preamble，brief 点名边界词 guard ⑬ WARN），出口用 file:line、confidence 与失败探针过滤。
