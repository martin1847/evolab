...
- **配比轴（真路径优先）**：治的病是**测试面冒充验证面**——单测与门禁堆量增长、真实用户路径
  从没人走过，最后主理人上手一试抓出一串 bug。开批前先判面：受益者是项目对外承诺服务的那类人 =
  业务面，是我们自己（席位 / 流程 / 工具 / 门）= 工程面，两者都真交付 = 混合面；**分类以落地物
  路径为准**（对外可交付面 vs 自用面），代理判错可覆写但须留一行理由。
  **自动闸咬结果停滞、不咬投入计数**（批的边界由执行者自划，按批数计的闸能被合批规避）：
  **距上次「真实用户路径被走通」超过本项目声明的阈值 → STOP-and-report**，冒泡「下一条要验的
  真路径是什么」再定，不自驱续投。比值（默认旋钮 8:2，是惯例起点不是发现）只在复盘记账呈现、
  由主理人每期调；连续 N 批纯工程面为廉价 advisory 兜底。工程批各自健康（评审收敛 / 规模锚绿）
  不构成配比正当。
...
- **回执要答「谁走的」，不是「跑了多少」**：一份完成回执必须能回答**谁、在真实模式下、走到第几步、
  看到了什么**；只答得出用例条数与 exit code 的回执 = 没人走过，按未完成收。推论三条：①「未验证项」
  里出现真实依赖 / 真实用户路径 = SHIP-BLOCKING，不许 accept-documented 放行；②被禁用的门必须在
  回执里作红项出现，不能只活在代码注释里；③真路径 runbook 跑不通 = 没人走过 = 未完成。
- **发现「绿而路没走」时，先去走那条路，别先加门**：加门是本能，而它把下一次发现推得更远——
  机制测试的增长与用户路径覆盖的增长互不产生因果，加多少条都不会让某条真路径第一次被走通。
