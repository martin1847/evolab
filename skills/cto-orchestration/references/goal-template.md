...
> <用户名> approval. Paths relative to `<server root>`. 如属研究：RESEARCH ONLY — no code changes, no commits.
...
- **动手前理解门**：先复述"碰哪些文件/契约、有哪些风险、scope 是什么"，等编排者放行再动手；
  高风险任务（鉴权/迁移/基建）升级为先交 mini-plan 等批准。
