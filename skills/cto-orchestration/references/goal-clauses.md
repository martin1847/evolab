...
> goal-template §场景条款 只留 16 行索引，条款正文在此单源。用法：逐条判命中 → 命中则把该条正文
...
- [ ] 本 goal 服务于一个父 Goal（总 Goal / 波次）→ 抬头写 `Parent-Goal: <绝对路径>`，Done-when 末条写
  **交付终点**：本 goal 到哪一步为止（集成 / 本机真路径 / 部署 / 线上验收），后续步骤归父 Goal、
  由编排位接续——worker DONE 不是波次的停点，父 Goal 状态由编排位回写。

## C16 证伪型 goal（spike / 对照 / 候选比较）的采用与弃用
- [ ] 本 goal 是证伪型（spike / 对照 / 候选比较）→ **采用 / 弃用条件开工前写死**（阈值与判据进
  Done-when，事后不改）；回执必答「采用 / 弃用」二选一，弃用保留负结果与读数，不算失败，
  也不得为交付把实验结果强塞进产品路径。
