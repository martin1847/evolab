...
- 被阻塞、想扩张 scope、或缺失信息**会改变目标 / 权限 / 验收判据**时：**停下并上报**，不自行授权、不合理化猜测；可逆的实现细节自决并记一行理由。
- 除非修复已获明确批准，否则不改既有用户可见行为；获批的变更直接落地，要逃生舱就加 kill switch（默认开、`=0` 可杀），不做默认 OFF→ON 两步翻转；只有 goal 标明**需灰度**（有生产流量且不可即时回滚）的变更才走 rollout 旗标默认 OFF（定义处写毕业条件）。
