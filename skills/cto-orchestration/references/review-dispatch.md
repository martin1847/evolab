...
## goal-review（派发前，动态触发）

> 触发判据：goal **定义新仪器**——验收判据、门、指标、停止规则、error 分型。命中即派发前加
> 1 轮冷上下文 goal-review（异构模型，只读 goal 文本，不喂实现语境）；取证 / 机械 / 纯研究类
> goal 免评，由 goal 内前提 CONFIRMED/REFUTED 表兜底。实证：贵的 goal 错误全部集中在仪器定义类
> （"FP 无害"只问伤害不问收益消失、用看不见截断的指标验收截断修复、停止规则未预声明即作废）；
> 取证类 goal 零事故——前提表在执行期抓错已够。

四问 checklist（任一问不过即 blocking，修 goal 再派）：
