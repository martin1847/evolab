...
>   中性指针（对象：该评审任务；判定面收窄实证 = prompt 文本本身——n=3 失败均为同会话**内联**
>   改写、n=1 同会话指针化一次通过〔2026-08-05〕；同会话指针化仍被拦 → 开新会话兜底；失败轮
>   照常计入轮数预算）。
...
>
> - [ ] 纯只读取证 / 调研 / 研究类（不写任何东西、只产出结论）→ 免评（判据二值可观察；
>   前提 CONFIRMED/REFUTED 表执行期兜底，实证零事故）
> - [ ] 其余——含一切带写动作的 goal，哪怕看着机械 → 1 轮冷上下文 goal-review（异构模型，
>   只读 goal 文本，不喂实现语境）；歧义即 fail-closed
>
> 原「机械执行类」免评条目已删（owner 裁 2026-08-06，外部席位 n=3 + 险误免评 n=2）：门禁 /
> 量具类天然伪装成机械活——改断言 / 棘轮 / 锚点即改判据，失效模式是**静默失明**（不报错，
> 只是不再报错，实现后评审与 CI 均不可救）；「像不像机械」是开放判断、「只读吗」是二值检查，
> 白名单只留后者，免评判断成本随之归零。评审在这类 goal 上最值钱的位置：**照抄先例时，
> 先例的选址 / 选型理由在新场景是否仍成立**——作者从先例出发思考，恰恰最难自查这条。
>
> 方向裁决（owner 2026-08-02，倾质量）：旧「触发列表」在开放集上枚举，漏一类 = 实现后才被拦、
> 烧 1-2h 实现轮（外部席位一日 n=4 实证，契约类设计缺陷全部纸面 10 分钟可拦而触发不命中）；
> 白名单在闭集上枚举，错一侧 = 多 10 分钟纸面。枚举放哪侧，决定失败方向朝哪边倒。

问块按 goal 命中的面选用，任一问不过即 blocking，修 goal 再派；命中仪器面时该轮还须产出
**承诺面变异清单**（异源出题，实现者逐条做到红——判据见仪器第 5 问）：

**仪器五问**（goal 定义验收判据 / 门 / 指标 / 停止规则 / error 分型时。实证：贵的 goal 错误
全部集中在仪器定义类）：
...
5. 坏样本**从哪来**？只认打在**承诺面**的（读者会信的那句话，破它即行为真变坏）；打在实现中间结构
   ...
   永远测不出（实证：契约测试四枚变异全打在表上，改真实行为后全套照绿，异构模型换个打法一击即穿）。
...
**shipped 脚本小改走轻量单轮**（owner 2026-07-30 立档，治「每次发布评审太重」）：文案/常量/单函数收紧、
匹配逻辑面未扩 → 单轮冷评审（--max-rounds 1）+ 机器可验修复即收，轮数耗尽转 owner 裁决；碰匹配
逻辑/新增规则/生命周期 → 完整对抗循环。实证：单轮腿照样抓出 P1（绝对路径绕过），轻量 ≠ 走过场。
