...
**承诺面坏样本清单**（异源出题，advisory、≤3 条，实现者只需其中 1 条真红 = 三态门的坏样本项；不得写成 binding 逐条复现表，功能验收测试不出变异表——判据见仪器第 5 问）；命中测量面
...
3. 负对照在哪（好样本必须绿）？测试面同问：去掉功能贡献时头号断言会不成立吗（阴性对照用例，goal-template §Done-when；只问头号断言，不铺到每个测试）？
...
出口滤（边界不构造变体、口径归 preamble；置信过滤放 verdict 层）。这里只留轴的枚举与装配表。
