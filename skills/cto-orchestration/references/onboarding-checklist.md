...
3. **wire 强制层 hooks**：用户级 settings 已有同名 entry 则项目级不重复接（同第 6 步 mail 判据）；缺则把 `cto-guard-bash.py` + `cto-guard-agent.py` + `cto-guard-edit.py` 三条 entry 并进 bootstrap §11 已建的
