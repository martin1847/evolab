...
让它自己找问题；人设 / 派发 brief / 筛选规则 / 输出契约 / 两轮法的机制本体在独立伴随 skill `frontend-explorer`，此处不复制。

## 本地起前端：坑项目特定 → 写进该项目 AGENTS.md

每个前端项目本地起服务的坑差别很大，不在此固化。项目 `AGENTS.md` 建一节写清四件事：后端代理目标与
控制它的 env（未设常请求 pending 挂死）、运行时配置注入点与临时覆盖法、登录是否多步（密码提交 ≠ 登录成功）、
测试运行器怪癖（如 `watch` 模式撞 `EMFILE` → 单跑兜底）。
唯一跨项目手法：要临时改硬编码的运行时配置又不污染工作树，在页面加载前用 `page.addInitScript`
（`playwright-cli run-code` 或 spec 里直接调）把配置对象 `Object.defineProperty(..., {writable:false})`
锁住，让页面自身的注入脚本覆盖失败；加载后求值的 `evaluate` 抢不到页面注入之前。

## 新项目接入

把「联调铁律」与「工具选型」写进该前端项目 `AGENTS.md`（"前端联调/浏览器运行时验证规约"节），
并钉死 `@playwright/cli`（仍在 0.x，命令面会漂）与 `@playwright/test` 的版本。
