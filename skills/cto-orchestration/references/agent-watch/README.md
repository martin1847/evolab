...
- **steer 语义**：默认排队（omp `follow_up` / claude 原生排队到下一 turn 边界）；`--now` 即时
  （omp `steer`；claude 无公开中断帧→降级排队并明说）；`--replace` 弃当前重来——**仅 omp**
  （`abort_and_prompt`）；claude 直接拒绝（降级会让注定作废的 turn 继续跑）并指路
  `stop` + `--resume <sid>` 重启，codex round 轮间本无活跃 turn、等价默认 steer。
  投递成功 ≠ 模型照做，验收仍看交付物。
