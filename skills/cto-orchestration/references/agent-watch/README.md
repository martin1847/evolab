...
- **`cto-guard-bash.py`（PreToolUse·Bash，deny = exit 2 + stderr）** — ① 拦尾随 `&`/`& disown`（双重后台 → 孤儿 +
  误判失联；`&&`/`2>&1`/引号内 `&`/前台 全放行）；② DENY「纯靠 idle-absence、无正向 grep」的裸轮询（idle≠done，分阶段
  commit 每个 part 边界都 idle 一瞬 → 误判；带 git 交付物核验 / pane 抓 Verdict·prompt 才放行）。
