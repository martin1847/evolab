...
- **`cto-guard-bash.py`（PreToolUse·Bash，①②=deny exit 2+stderr，③=提醒 additionalContext）** — ① 拦尾随 `&`/`& disown`
  （双重后台 → 孤儿 + 误判失联；`&&`/`2>&1`/引号内 `&`/前台 全放行）；② DENY「纯靠 idle-absence、无正向 grep」的裸轮询
  （idle≠done，分阶段 commit 每个 part 边界都 idle 一瞬 → 误判；带 git 交付物核验 / pane 抓 Verdict·prompt 才放行）；
  ③ `dispatch <agent> <session>` 后**没在同条命令里 arm `watch <session>`** → 注入提醒（漏挂 watcher = 退化成裸计时器
  猜，是 omission 无法硬 deny；同条已带 watch 则静默）。
- **`cto-guard-agent.py`（PostToolUse·Agent|Task）** — browser/E2E subagent 派发那刻注入 deadline-watch 提醒（完成通知
  会黑洞、omission 无法硬 deny，只能 launch 时强制 salience）。**提醒 = JSON `hookSpecificOutput.additionalContext`**——
  PostToolUse 纯文本 stdout 只进 debug log、agent 看不到，必须走 JSON 才送达。
...
**不另造 settings 脚手架——并进 `repo-governance-bootstrap` §11 已建的那份**（三调度者 wiring 模式 + `<S>`=skill 安装路径
约定同源于它；hooks 不展开 `~`，`<S>` 用绝对/`${CLAUDE_PROJECT_DIR}`）。cto 只加这两条 entry（脚本可执行 + shebang，
**直接 exec、别加 `python3 ` 前缀**）：
```jsonc
// .claude/settings.json（codex 同 JSON 放 .codex/hooks.json）
"hooks": {
  "PreToolUse":  [{ "matcher": "Bash",        "hooks": [{ "type": "command", "command": "<S>/references/agent-watch/cto-guard-bash.py" }] }],
  "PostToolUse": [{ "matcher": "Agent|Task",  "hooks": [{ "type": "command", "command": "<S>/references/agent-watch/cto-guard-agent.py" }] }]
}
```
