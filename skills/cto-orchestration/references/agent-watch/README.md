...
- **`cto-guard-bash.py`（PreToolUse·Bash；deny=exit 2+stderr，提醒=additionalContext）** — ① 拦背景 `&`（**剥引号 span 后
  ...
  不误报），④ 用原始 cmd（CJK 在引号内）。**git-push 治理不在此**——归 Git Workflow skill `im-git-workflow` + 服务端分支保护（IaC ADR-0012）。
...
"hooks": {
  "PreToolUse":  [
    { "matcher": "Bash",             "hooks": [{ "type": "command", "command": "<S>/references/agent-watch/cto-guard-bash.py" }] },
    { "matcher": "Agent|Task|TaskStop", "hooks": [{ "type": "command", "command": "<S>/references/agent-watch/cto-guard-agent.py" }] }
  ],
