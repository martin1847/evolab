...
你想让 AI 替你把活全干了，于是开了个 agent 一把梭——结果它**声称完成但没 commit**、它**自己评审自己说没问题**、CI **全绿但产物早就停更**。单 agent 一把梭是公认反模式，多 agent 又乱成一团：谁派谁、谁信谁、死了怎么知道？

缺的从来不是工具，是**纪律**。

`cto-orchestration` 把一套在真实多 agent 项目里打磨出来的编排纪律沉淀成 skill：编排者**本人绝不写产品代码**，只产出 goal 文档、监控、评审调度、决策落盘；执行交给一个 agent、评审交给**另一个不同模型**的 agent 做对抗式复核；watcher 后台盯**存活信号**（agent 死了退回 shell ≠ 任务完成）；行为变更藏旗标后默认零变化。
...
这是核心设计，也是它和别人最不一样的地方——**三个位置，刻意用不同强度/不同家族的模型**：
...
| watcher 状态 | typed 信号：DONE / SESSION-GONE / AGENT-DEAD / HANG / WAITING-INPUT |
...
- **不可逆操作先报后做**：`push` / PR / 迁移 / 删除 / 对外消息——先说意图 + 影响面，等明确放行。本地 commit 留着，push 必须用户明示批准，一次只覆盖那一次。
- **旗标门控**：行为变更默认藏 env flag 后，默认 OFF = 字节级零变化。
- **验证诚实三段式**：每次交付说清——验证了什么（真跑过）/ 没验证什么 / 剩余风险。本地打桩绕过的环节显式标注。
- **不写产品代码**：编排者只派工、监控、评审、落盘，绝不亲自下场改产品代码。

## 文件结构

```
cto-orchestration/
├── SKILL.md                          # 给 agent 看的完整编排方法论（8 节派工协议）
├── README.md                         # 本文件：给人看的安装+定位页（图存仓库根 assets/，不随 skill 安装）
└── references/
    ├── goal-template.md              # 派工 goal 文档模板
    ├── watcher.sh                    # 后台轮询 + 存活检测脚本
