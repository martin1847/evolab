...
$AGENT_MAIL_DIR/            # 默认 ~/.agents/mail（与 agent-watch 的 ~/.agents/run 同规）
  registry.md               # 名册：id → 工作目录 → 职责（agent-bus register 维护）
...
**`agent-bus` helper 已 `umask 077` 强制**——它建的目录 700、文件 600，无需事后 chmod（结构层，非纪律）；
...
2. **wire hooks**：**entry 真源 = 本 skill `hooks.json`（别抄散文）**——读它、command 换安装根
   ...
   - **codex / omp 无此能力**：codex 事件集只有 `PreToolUse`/`PermissionRequest`/`Stop`（无 SessionStart /
     ...
     长跑席位收中途信靠 `agent-bus check` 主动查（或该席位由 CC 编排者代管转达）。
