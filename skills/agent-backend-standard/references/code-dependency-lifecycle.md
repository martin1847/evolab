...
"行为变更藏 env flag、默认 OFF = 字节级零变化"是 **goal / owner 标明需要灰度的生产变更**的好**出生**纪律(安全 ramp;其余获批变更直接落地,逃生舱用 §3.1 的 kill-switch,不做 OFF→ON 两步翻转);但只写出生、不写**死亡**,旗标只增不减 → rollout 旗标的 OFF 分支变成没人跑没人测的死代码,随代码漂移腐烂(某天真去回滚,逃生舱已坏 = illusory rollback);不设毕业期限时 rollout 旗标可能永不毕业,零 reader 的死旗标定义还挂着——旗标膨胀是每个服务的工程卫生债(同 §0 那个"没人清的 ES")。
...
- **kill-switch**:故意保留的永久安全/禁用开关。保留。获批变更的逃生舱(默认开、`=0` 可杀:未设或非 `0` 即开,解析走仓统一 helper)亦归此类,死旗标扫描同查 `=0` 路径仍可用;稳定后可按 rollout 规则删。
