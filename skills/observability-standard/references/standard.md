...
- **数据两档**:只有跨进程数据(JSON / HTTP 载荷、manifest、配置)用 **Pydantic v2** `BaseModel`,`model_config = ConfigDict(extra="forbid", frozen=True)`(抓 typo + 边界对象不可变),边界 `model_validate` 一次;进入内部即转 `@dataclass(frozen=True, slots=True)`,内部 DTO(事件 / 信封 / 结果)不带 Pydantic 依赖与校验开销。混用模块在模块级注释写明边界在哪一行。
- `NewType` 区分 ID:`RunId = NewType("RunId", str)`。
- `Literal` 表状态/角色/事件名。
- 结果用判别联合(内部 → frozen dataclass):`Ok` / `Err` 各一个 `@dataclass(frozen=True, slots=True)`,以 `status: Literal["ok"]` 判别,`Result = Ok | Err`,调用方 `match`。
- 接口用 `typing.Protocol`(结构化子类型)不用 ABC;默认实现体放独立 mixin 类(只结构化符合、不显式继承协议的类拿不到 Protocol 里的默认实现)。
...
> 事实基线(2026-07 亲验):`gen_ai.*` **零属性 Stable**(SEO 站"已稳定"说法与 spec 原文矛盾,勿信);约定 2026-06-12 整体迁仓至 `open-telemetry/semantic-conventions-genai`(旧 semconv 固定链接/codegen 失效);`gen_ai.system` 已废、现名 `gen_ai.provider.name`(主流平台仍读写旧名,接入逐属性核对);**Java 上游 GenAI instrumentation 覆盖窄**(官方 supported-libraries 矩阵仅 OpenAI Java SDK 与 AWS SDK 两族发 GenAI spans/metrics)——矩阵外 provider 的 `gen_ai.*` 需自建(pin revision + 内部抽象层,字面量不散落业务代码),接入前先查 pinned 矩阵。
>
...
**核心命题:没有 gate 的规范 = 形式化,必然漂移。** 一份只写"应当"、靠人工自觉对照清单的规范,会在"没有测试看的地方"悄悄烂掉——埋点的洞恰好出现在没人 gate 的模块,评审与单测全绿、"看着合规",直到线上"查不到任何 trace"才暴露。**强制手段是采纳可观测性的一等交付物,不是事后补丁。**
...
- 把 conformance + 类型门禁(§8)接进 CI,**核它真触发**:workflow 放在工具识别的位置(GitHub Actions 只跑**仓库根** `.github/workflows/`,放子目录=静默不跑)、paths filter 覆盖所有该触发的改动、步骤**无 `|| true` / 无 soft-fail**。**在一个真 PR 上验证 gate 确实触发并能挂,别假设。**
