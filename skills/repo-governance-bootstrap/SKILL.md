...
11. **（可选）配置 memory-discipline hook**：**先征询用户**（纯 opt-in，不愿可跳）。要配则在 settings.json
    （全局 `~/.claude/` 覆盖所有项目 / 项目 `.claude/` 仅本仓，问用户）加下方「memory-discipline hook 模板」的
    PostToolUse hook：写 `memory/*.md` 时确定性注入"事实细节→ACCESS.local.md/docs、只留指针"提醒。**为什么**：
    该纪律在 `cto-orchestration` §5（知识层），但 skill 文本随长对话 salience 衰减，高频纪律须 hook 强制层兜底。
    格式按你的 Claude Code 版本校验；**绝不**把真实 secret 写进 hook。
...
## memory-discipline hook 模板（步骤 11，可选）

> PostToolUse hook：写 `memory/*.md`（非 `MEMORY.md` 索引）时注入"细节→环境文档、只留指针"提醒。
> 强制层（确定性触发），补 `cto-orchestration` §5 知识层文本的 salience 衰减。格式按 CC 版本校验。

```jsonc
// settings.json（全局 ~/.claude 或项目 .claude）
"hooks": {
  "PostToolUse": [{
    "matcher": "Write|Edit|MultiEdit",
    "hooks": [{
      "type": "command",
      "command": "f=$(jq -r '.tool_input.file_path // empty'); case \"$f\" in */memory/*.md) [ \"$(basename \"$f\")\" != MEMORY.md ] && echo '{\"hookSpecificOutput\":{\"hookEventName\":\"PostToolUse\",\"additionalContext\":\"memory 写入提醒：事实细节(schema/config/creds/endpoint/长清单)→ACCESS.local.md/docs；memory 正文只留指针+教训。\"}}' ;; esac"
    }]
  }]
}
```
