...
> §1–§2 的全流程。git/agentctl 命令是真实形态（参数按你项目校准），内容是构造的、
...
## 2. agentctl 派发 omp + 挂 watcher

```bash
AGENTCTL=skills/cto-orchestration/references/agentctl/agentctl
"$AGENTCTL" start omp demo-ratelimit-omp ../wt-ratelimit \
  --goal /abs/path/docs/orchestration/RATELIMIT_GOAL.md
# 挂 watcher（协议主信号，阻塞到终态、返 typed 退出码；Claude Code 里用 Bash run_in_background）
"$AGENTCTL" watch demo-ratelimit-omp; rc=$?
```

watcher 输出（typed 状态）：

```
[demo-ratelimit-omp] heartbeat — RUNNING: turn in progress   # 仍在跑、存活
=== [demo-ratelimit-omp] DONE: engine idle                    # 终态（活着 + 空闲）
```

> watcher 是 typed 终态：`AGENT-DEAD` ≠ `DONE`——死会话别派去评审。
...
```bash
"$AGENTCTL" start codex demo-ratelimit-codex ../wt-ratelimit \
  --goal /abs/path/docs/orchestration/RATELIMIT_REVIEW_BRIEF.md \
  --workflow review-loop --max-rounds 3 --no-preflight   # 评审属取证类，显式豁免 preflight
"$AGENTCTL" watch demo-ratelimit-codex
```
