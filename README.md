...
| 目录 | 是什么 |
| --- | --- |
| **`skills/`** | 可直接装进 Claude Code 的 skill。当前：`cto-orchestration`（**旗舰**——多 agent 编排）、`repo-governance-bootstrap`（文档治理）、`observability-standard`（可观测性）、`git-workflow-standard`（Git 协作 SOP）、`agent-backend-standard`（agent 后端工程手册）。后续更多。 |
