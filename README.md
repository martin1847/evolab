...
| 目录 | 是什么 |
| --- | --- |
| **`skills/`** | 可直接装进 Claude Code 的 skill。当前：`cto-orchestration`（**旗舰**——多 agent 编排）、`repo-governance-bootstrap`（文档治理）、`agent-mail`（编排者间信箱总线，cto 的可选伴随）、`observability-standard`（可观测性）、`git-workflow-standard`（Git 协作 SOP）、`agent-backend-standard`（agent 后端工程手册）、`source-coding-discipline`（写码纪律的 skill 形态，给不支持条件 rules 的 agent，如 codex）。后续更多。 |
...
| **`meta/`** | 元认知：可逆性分配 agency、验证诚实性、先测再加（measure-before-more）、结构而非自律（structure-not-discipline）、配置接线与治理（wiring）。**灵魂层**——我是怎么想的。 |

## 用起来

skill 本质是 markdown + 几个脚本——**AI-native 装法：把下面这段（含仓库地址）直接丢给你的 agent，它自己读懂 + 装好。** 任何能读项目规则的 agent 都行，不限 Claude Code。

> 帮我 clone https://github.com/martin1847/evolab ，先读一遍 `README.md` 和 `meta/` 理解核心理念 A²——
> 一个编排者调度一群专家。然后把需要的 `skills/<name>` **整个目录（含 `references/`）复制**进我的
> skills / 规则目录（Claude Code 放 `~/.claude/skills/`；其他 agent 放对应 rules / skills 目录）：
> **必装** `cto-orchestration`（多 agent 编排）+ `repo-governance-bootstrap`（仓库治理）；
...
> codex 等不支持条件 rules 的 agent 改装 `skills/source-coding-discipline`（同一纪律的 skill 形态，
> 与 rules 形态二选一，勿同装）。
...
evolab 会持续进化——也是阳哥从"单打独斗"往开放协作迈的第一步。

- 觉得有用，点个 ⭐ 是最轻的支持，也让更多人找得到它。
- 有想法 / 踩到坑 / 想补个 skill：欢迎开 issue 或提 PR，一句话也行。
